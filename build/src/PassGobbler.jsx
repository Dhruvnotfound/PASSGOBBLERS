...
class DataFetcher extends React.Component {
  ...
  componentDidMount() {
    fetch("https://api") //your api here //change
      .then((res) => {
        if (!res.ok) {
...
const PassGobbler = () => {
  ...
  const setupDynamoDBClient = (data) => {
  ...
  const fetchPasswords = async (data) => {
    const docClient = setupDynamoDBClient(data);
    const params = {
    ...
    try {
      ...
      const decryptedPasswords = result.Items.map(item => ({
        ...item,
        password: decryptPassword(item.password, data.ENCRYPTION_KEY)
      }));
  ...
  const addPassword = async (data) => {
    if (newSite && newUsername && newPassword) {
      const encryptedPassword = encryptPassword(newPassword, data.ENCRYPTION_KEY);
      const newItem = {
        site: newSite,
  ...
  const confirmDelete = async (index, data) => {
                    ...
                    <div className="flex justify-between items-center mb-2">
                      ...
                      {deleteConfirmation[index] ? (
                        <div className="flex items-center">
                          ...
                          <button
                            onClick={() => confirmDelete(index, data)}
